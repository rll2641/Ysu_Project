...
def shortest_route_dfs_1(start, end):
    ...
    for line in line_list:
        ...
        while left >= 0 or right < len(st_list) - 1:
            
            if left < 0: left = 0 
            if right > len(st_list) - 1: right = len(st_list) - 1
...
def shortest_route_dfs_2(start, end, ed_line):
    ...
    for st_line in start_line_list:
        ...
        while left >= 0 or right < len(start_line) - 1:
            
            if left < 0: left = 0 
            if right > len(start_line) - 1: right = len(start_line) - 1
            ...
            try:
                ...
                for line in si.st_infor.get(start_line[left]).get('호선'):
                    if line not in si.st_infor.get(end).get('호선') and line not in si.st_infor.get(start).get('호선'):
                        ...
                        for stnm in st_list:
                            ...
                            left_tr_1 = shortest_route_dfs(start_line[left], stnm, line)
                            left_tr_1.pop()
                            tr_2 = shortest_route_dfs(stnm, end, ed_line)
                            left_route = left_visted + left_tr_1 + tr_2
                ...
                for line in si.st_infor.get(start_line[right]).get('호선'):
                    if line not in si.st_infor.get(end).get('호선') and line not in si.st_infor.get(start).get('호선'):
                        ...
                        for stnm in st_list:
                            right_tr_1 = shortest_route_dfs(start_line[right], stnm, line)
                            right_tr_1.pop()
                            tr_2 = shortest_route_dfs(stnm, end, ed_line)
                            right_route = right_visited + right_tr_1 + tr_2 
            ...
            if start_line[left] not in left_visted:
                left_visted.append(start_line[left])
            if start_line[right] not in right_visited:
